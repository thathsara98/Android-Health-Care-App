...
public class DBdata extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String query2 = "create table cart(username text, product text, price float,otype text)"; //order type
        db.execSQL(query2);

        String query3 = "create table orderPlace(username text, fullname text, address text, contactNo text, pincode text, date text, time text, amount float,otype text)"; //order type
        db.execSQL(query3);
    }
    ...
    public int login(String username, String password) {
        ...
        str[0] = username;
        str[1] = password;
        SQLiteDatabase db = getReadableDatabase(); //match the data
    ...
    public ArrayList getCartData(String username, String otype){
        ...
        if(c.moveToFirst()){
            ...
            }while(c.moveToNext()); //do while using because there maybe have more than 1 cart items
        }
        db.close();
        return arr;
